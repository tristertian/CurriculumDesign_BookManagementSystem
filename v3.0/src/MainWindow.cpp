...
MainWindow::MainWindow(int width, int height, const char* title) 
    ...
    salesSystem = new SalesSystem(bookManager);
    statsSystem = new StatisticsSystem(bookManager);
...
void MainWindow::handlePurchaseBook() {
    ...
    if (salesSystem->purchaseBook(isbn, quantity)) {
        updateTable();
        
        double total = salesSystem->getTotalPrice(isbn, quantity);
        std::stringstream ss;
        ss << "购买成功！\nISBN: " << isbn << "\n数量: " << quantity 
...
void MainWindow::handleSave() {
    if (bookManager->saveToFile("data/books.dat")) {
        showMessage("数据保存成功！\n文件位置: data/books.dat");
    } else {
        showError("数据保存失败！");
    }
...
void MainWindow::handleLoad() {
    if (bookManager->loadFromFile("data/books.dat")) {
        updateTable();
        showMessage("数据加载成功！\n共加载 " + 
                   std::to_string(bookManager->getBookCount()) + " 本图书");
    } else {
